...
void job_Init()
{
    ...
    job_indices = create_stack_list(sizeof(uint32_t ), MAX_JOBS);
    job_pool = calloc(sizeof(struct job_t), MAX_JOBS);
    job_data_buffers = calloc(sizeof(struct job_data_buffer_t ), MAX_JOBS);

    for(uint32_t i = 0; i < MAX_JOBS; i++)
    {
        job = get_stack_list_element(&job_pool, i);
